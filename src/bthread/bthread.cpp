...
DEFINE_int32(bthread_current_tag, BTHREAD_TAG_INVALID,
             "Set bthread concurrency for this tag");
...
inline TaskControl* get_task_control() { return g_task_control; }

inline TaskControl* get_or_new_task_control() {
  butil::atomic<TaskControl*>* p =
      (butil::atomic<TaskControl*>*)&g_task_control;
...
BUTIL_FORCE_INLINE bool can_run_thread_local(
...
int bthread_equal(bthread_t t1, bthread_t t2) { return t1 == t2; }
...
int bthread_getconcurrency(void) { return bthread::FLAGS_bthread_concurrency; }
...
int bthread_setconcurrency(int num) {
  ...
  if (num > bthread::FLAGS_bthread_concurrency) {
    ...
    auto added = bthread::add_workers_for_each_tag(
        num - bthread::FLAGS_bthread_concurrency);
...
int bthread_list_init(bthread_list_t* list, unsigned /*size*/,
